...
本文提出的语音识别系统采用模块化设计，整个处理流程如图\ref{fig:processing_flow}所示。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[node distance=1.2cm, auto, scale=0.5]
    % 定义节点样式
    \tikzstyle{process} = [rectangle, minimum width=1.5cm, minimum height=0.6cm, text centered, draw=black, fill=blue!20, font=\tiny]
    \tikzstyle{decision} = [diamond, minimum width=0.8cm, minimum height=0.4cm, text centered, draw=black, fill=yellow!20, font=\tiny]
    \tikzstyle{startend} = [ellipse, minimum width=1.2cm, minimum height=0.6cm, text centered, draw=black, fill=green!20, font=\tiny]
    \tikzstyle{arrow} = [thick,->,>=stealth]

    % 定义节点
    \node (start) [startend] {开始};
    \node (input) [process, below of=start] {音频输入};
    \node (preprocess) [process, below of=input] {音频预处理};
    \node (window) [process, below of=preprocess] {窗函数处理};
    \node (feature) [process, below of=window] {特征提取};
    \node (endpoint) [process, below of=feature] {端点检测};
    \node (decision) [decision, below of=endpoint, yshift=-0.5cm] {是否检测到语音段？};
    \node (classify) [process, right of=decision, xshift=2cm] {分类识别};
    \node (output) [process, above of=classify] {输出识别结果};
    \node (end) [startend, above of=output] {结束};
    \node (continue) [process, left of=decision, xshift=-2cm] {继续检测};

    % 绘制箭头
    \draw [arrow] (start) -- (input);
    \draw [arrow] (input) -- (preprocess);
    \draw [arrow] (preprocess) -- (window);
    \draw [arrow] (window) -- (feature);
    \draw [arrow] (feature) -- (endpoint);
    \draw [arrow] (endpoint) -- (decision);
    \draw [arrow] (decision) -- node[anchor=south] {是} (classify);
    \draw [arrow] (decision) -- node[anchor=south] {否} (continue);
    \draw [arrow] (continue) -- (endpoint);
